...
<<<<<<< HEAD
              if(varproject.consultyNeeded){
                await Platform.create({
                    ...
                    description:varproject.description
           ...
           const platformItem= await Platform.create({
                   ...
                   const members = await User.find({ tags: 'Member', skills: { $all: varproject.requiredSkills } })
                   members.map((member) => {
                       member.recomended.unshift({
                           id:platformItem._id,
                           type: 'Project',
                           date: new Date().toJSON(),
                           owner: {
...
try{
    ...
    if(!project) return  res.json({ msg: 'This Project Dosent Exist' })
   ...
   if(project.projectPartner.id==ownerId||project.projectConsultancyAgency.id==ownerId){
    if(!member){
        project.appliedMembers.filter(member => member.id != memberId)
        project.save()
    ...
    sendTaskNotification(projectId, memberId, 'You have been assigned!')
    sendTaskNotification(projectId, project.partnerId, 'You project has been assigned to a member!')
    member.save()
    project.save()
    res.json({msg:'Accepted', data: project })
    }else{
        sendTaskNotification(projectId, memberId, 'You have been assigned!')
        sendTaskNotification(projectId, project.partnerId, 'You project has been assigned to a member!')
        member.save()
