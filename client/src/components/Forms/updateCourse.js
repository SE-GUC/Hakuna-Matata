...
export class updateCourse extends Component {
    ...
    componentDidMount() {
        const {id,courseId}=this.props.match.params
            ...
            axios
          .get(`http://localhost:3333/courses/${courseId}`)
          .then(res => {
    ...
    getData(){
      ...
      <hr style={lineStyle}></hr>  <p > name: <input type="text" placeholder={this.state.name}  onChange={e => this.nameChange(e)}/></p>
      <p > description: <input type="text" placeholder={this.state.description}  onChange={e => this.desChange(e)}/></p>
      <p > places: <input type="text" placeholder={this.state.places}  onChange={e => this.placesChange(e)}/></p>
      <p > availablePlaces: <input type="text" placeholder={this.state.availablePlaces}  onChange={e => this.avPlacChange(e)}/></p>
      <p > payment: <input type="text" placeholder={this.state.payment}  onChange={e => this.paymentChange(e)}/></p>
      <p > courseDuration: <input type="text" placeholder={this.state.courseDuration}  onChange={e => this.durChange(e)}/></p>
      <p > startDate: <input type="text" placeholder={this.state.startDate}  onChange={e => this.sdChange(e)}/> </p>
      <p > endDate: <input type="text" placeholder={this.state.endDate}  onChange={e => this.edChange}/></p>
      <p > category: <input type="text" placeholder={this.state.category}  onChange={e => this.catChange(e)}/></p>
    ...
    render() {
        ...
        if(this.state.updated){
            ...
            return(<Redirect to=  {"/course/"+courseId}  ></Redirect>)
