 ...
 render(){
   const mappingFunction = component => component+" |";
    ...
    return (
      ...
      <PhotoComponent KeyImage={KeyImage}/><br></br>
      <Button style={{position:'absolute',left:'38.5%'}}>Chat</Button><br/>
     ...
     <Data keyy={'Skills:'} value={this.state.skills.map(mappingSkills) }/><br/>
      ...
      <p>{this.state.partnerTasks.map(mappingJobs)}</p><br/>
      <p>{this.state.partnerProjects.map(mappingInfo)}</p><br/>
