...
class FirstComponent extends Component {
  get() {
    if(store.get('payload').tags.includes('EducationOrngaization')&&store.get('payload').id==this.props.id){
      return <div style={{ position: "absolute", left: "90%", width: 250 }}>
      <Row>
      <Link to={`/updateMasterCLass/${this.props.id}/${this.props.masterClass._id}` }> <button style={{backgroundColor:'transparent',border:'none'}}><img src={require("../../assessments/images.jpeg")} width="30px"
            height="30px"/></button>
