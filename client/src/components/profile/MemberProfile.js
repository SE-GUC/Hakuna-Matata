...
class MemberProfile extends Component {
  ...
  componentDidMount(){
    ...
    axios
    .get(`http://localhost:3333/masterclasses`)
    .then(res => {
...
if(returnedData.length ==0){
  returnedData.push(<div><h1>Not Applied To Tasks Yet</h1> <Link push to={"http://localhost:3000/tasks"}> View some tasks</Link>
  </div>)
}
...
if( this.state.Project){
  ...
  if(id==this.props.auth.user._id)
  if(returnedData.length ==0){
    returnedData.push(<div><h1>Not Applied To Projects Yet</h1> <Link push to={"http://localhost:3000/projects"}> View some projects</Link>
    </div>)
  }
}
