...
class ShowAllPartner extends Component {
    constructor(props) {
        super(props);
        this.state = {
          tasks:[]
    ...
    for(let x=0 ;x<array.length;x++){
      returnedData.push(
        <Card>
        <Card.Body>
          <Card.Title>{array[x].partnerName}</Card.Title>
          <div style={{
            position:'absolute',
            right:10
          }}>
          <Link to={`/partners/${array[x]._id}`} style={{ color: "black" }}>
          View
          </Link>
          </div>
        </Card.Body>
